...
class MetaDatasetExt extends MetaDatasetImpl {
    private static final Logger logger = LoggerFactory.getLogger(MetaDatasetExt.class);

    @Override
    String loadAllColumns() {
        def resource = DocFinder.create(Context.current.store, DatasetPackage.Literals.META_DATASET, [name: this.name])
                .execute().resourceSet
        if (!resource.resources.empty) {
            ...
            def metaDataset = resource.resources.get(0).contents.get(0) as MetaDataset
            if (resultSet != null) {
