                                            ...
                                            <Form.Item style={{ display: 'inline-block' }}>
                                                {getFieldDecorator(`${idDatasetColumn}`,
                                                    ...
                                                    <Select
                                                        ...
                                                        allowClear={true}
                                                        onChange={(e: any) => {
                                                            const event = e ? e : JSON.stringify({index: highlight.index, columnName: 'datasetColumn', value: undefined})
                                                    ...
                                                    >
                                                        {
                                                            this.props.columnDefs!
                                                                .map((c: any) =>
                                                                    <Select.Option
                                                                        key={JSON.stringify({index: highlight.index, columnName: 'datasetColumn', value: c.get('field')})}
                                                                        value={JSON.stringify({index: highlight.index, columnName: 'datasetColumn', value: c.get('field')})}
                                                                    >
                                                                        {c.get('field')}
                                                                    </Select.Option>)
                                                        }
                                            ...
                                            <Form.Item style={{ display: 'inline-block' }}>
                                                {getFieldDecorator(`${idOperation}`,
                                                    ...
                                                    <Select
                                                        ...
                                                        allowClear={true}
                                                        onChange={(e: any) => {
                                                            const event = e ? e : JSON.stringify({index: highlight.index, columnName: 'operation', value: undefined})
                                                            this.handleChange(event)
                                                        }}
                                                    >
                                                        {
                                                            this.props.allOperations!
                                                                .map((o: any) =>
                                                                    <Select.Option
                                                                        key={JSON.stringify({index: highlight.index, columnName: 'operation', value: o.get('name')})}
                                                                        value={JSON.stringify({index: highlight.index, columnName: 'operation', value: o.get('name')})}
                                                                    >
                                                                        {t(o.get('name'))}
                                                                    </Select.Option>)
                                                        }
                                                ...
                                                {getFieldDecorator(`${idValue}`,
                                                    ...
                                                    <Input
                                                        ...
                                                        onChange={(e: any) => this.handleChange(
                                                            JSON.stringify({index: highlight.index, columnName: 'value', value: e.target.value === "" ? undefined : e.target.value})
                                                        )}
                                            ...
                                            <Form.Item style={{ display: 'inline-block' }}>
                                                {getFieldDecorator(`${idEnable}`,
                                                    ...
                                                    <Select
                                                        ...
                                                        style={{width: '82px'}}
                                                        onChange={(e: any) => {
                                                            const event = e ? e : JSON.stringify({index: highlight.index, columnName: 'enable', value: undefined})
                                                            this.handleChange(event)
                                                        }}
                                                    >
                                                        <Select.Option
                                                            key={JSON.stringify({index: highlight.index, columnName: 'enable', value: false})}
                                                            value={JSON.stringify({index: highlight.index, columnName: 'enable', value: false})}
                                                        >
                                                            {t('false')}
                                                        </Select.Option>
                                                        <Select.Option
                                                            key={JSON.stringify({index: highlight.index, columnName: 'enable', value: true})}
                                                            value={JSON.stringify({index: highlight.index, columnName: 'enable', value: true})}
                                                        >
                                                            {t('true')}
                                                        </Select.Option>
                                                    </Select>
                                        ...
                                        <Row>
                                            <Col span={4} style={{marginLeft: '17px', marginTop: '-17px' }}>
                                                {getFieldDecorator(`${idHighlightType}`,
                                                    {
                                                        initialValue: highlight.highlightType
                                                    })(
                                                    <Select
                                                        allowClear={true}
                                                        style={{width: '100px'}}
                                                        onChange={(e: any) => {
                                                            const event = e ? e : JSON.stringify({index: highlight.index, columnName: 'highlightType', value: undefined})
                                                            this.handleChange(event)
                                                        }}
                                                    >
                                                        {
                                                            this.props.allHighlightType!
                                                                .map((o: any) =>
                                                                    <Select.Option
                                                                        key={JSON.stringify({index: highlight.index, columnName: 'highlightType', value: o.get('name')})}
                                                                        value={JSON.stringify({index: highlight.index, columnName: 'highlightType', value: o.get('name')})}
                                                                    >
                                                                        {t(o.get('name'))}
                                                                    </Select.Option>)
                                                        }
                                            ...
                                            <Col span={8} style={{marginRight: '20px', marginLeft: '20px', textAlign: 'center', marginTop: '-17px'}}>
                                                <div style={{display: "inline-block", fontSize: '17px', fontWeight: 500, marginLeft: '18px', color: '#878787', marginRight: '20px'}}>Фон</div>
                                                <SliderPicker
                                                    onChangeComplete={(e: any) => {
                                                        const event = JSON.stringify({index: highlight.index, columnName: 'backgroundColor', value: e.hex})
                                                        this.handleChange(event)
                                                    }}
                                                    color={ highlight.backgroundColor === undefined ? 'white' : highlight.backgroundColor }
                                                />
                                            </Col>
                                            <Col span={8} style={{marginRight: '20px', marginLeft: '20px', textAlign: 'center', marginTop: '-17px'}}>
                                                <div style={{display: "inline-block", fontSize: '17px', fontWeight: 500, marginLeft: '18px', color: '#878787', marginRight: '20px'}}>Текст</div>
                                                <SliderPicker
                                                    onChangeComplete={(e: any) => {
                                                        const event = JSON.stringify({index: highlight.index, columnName: 'color', value: e.hex})
                                                        this.handleChange(event)
                                                    }}
                                                    color={ highlight.color === undefined ? 'white' : highlight.color }
                                                />
                                            </Col>
                                        </Row>
                                    </Form.Item>
